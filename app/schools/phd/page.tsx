...
import { Search, ArrowRight, GraduationCap, BookOpen, Rocket, X, FlaskConical } from "lucide-react";
...
export default function PhDProgrammesPage() {
  ...
  /* ─── get only PhD courses ─── */
  const phdCourses = useMemo(
    () => getAllCourses().filter((c) => c.type === "PhD"),
    [],
  );

  /* ─── schools that have PhD programmes ─── */
  const schools = useMemo(() => {
    const slugsWithPhD = new Set(phdCourses.map((c) => c.schoolSlug));
    return [
      { slug: "all", label: "All Schools" },
      ...SCHOOL_ORDER.filter((s) => slugsWithPhD.has(s.slug)),
    ];
  }, [phdCourses]);
  ...
  const stats = [
    { value: "10",              label: "Research Schools" },
    { value: "3–5 Yrs",        label: "Programme Duration" },
    { value: "60+",             label: "Research Areas" },
    { value: "Ph.D.",           label: "Degree Awarded" },
  ];

  const icons = [<BookOpen size={18} />, <GraduationCap size={18} />, <Rocket size={18} />, <FlaskConical size={18} />];
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-[#f5f7fa] py-3">
        <div className="mx-auto flex max-w-[1400px] flex-wrap px-4 text-xs text-gray-600 md:px-6 md:text-sm">
          ...
          <span>Schools</span>
          <span className="mx-2 text-gray-400">›</span>
          <span className="text-[#0A8F96] font-medium">PhD & Research</span>
        </div>
      </div>
      ...
      <section className="relative w-full overflow-hidden min-h-[860px] md:min-h-screen">
        ...
        <div className="relative z-10 mx-auto flex min-h-[860px] max-w-[1400px] flex-col justify-end px-4 pb-8 pt-28 md:h-full md:min-h-screen md:justify-center md:px-14 md:pb-0">
          <div className="max-w-[560px]">
            ...
            >
              Pursue cutting-edge doctoral research across 10 schools at Presidency University.
              Join a vibrant research community with expert supervisors, modern labs, and
            ...
            {schools.map((s) => (
              <button
                ...
                className={`shrink-0 px-4 py-2 rounded-lg text-xs transition-all ${
                  activeSchool === s.slug
                    ? "bg-[#6b4f9a] text-white shadow-sm"
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-10 pt-8 pb-2">
        <p className="text-sm text-gray-500">
          Showing <span className="text-[#6b4f9a]">{filtered.length}</span> programme{filtered.length !== 1 ? "s" : ""}
          {activeSchool !== "all" && (
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-10 py-6 pb-20">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-gray-400">
            ...
            <button onClick={() => { setQuery(""); setActiveSchool("all"); }} className="mt-4 text-[#6b4f9a] text-sm hover:underline">
              Clear filters
            </button>
          </div>
        ) : grouped ? (
              ...
              <div key={slug}>
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3">
                    <div className="w-1 h-6 rounded-full bg-[#6b4f9a]" />
                    <h2 className="text-lg text-[#1e3a5f]">{label}</h2>
                    <span className="text-xs bg-[#6b4f9a]/10 text-[#6b4f9a] px-2.5 py-1 rounded-full">{courses.length}</span>
                  </div>
      ...
      <section className="bg-gradient-to-r from-[#0F1E3D] to-[#0D3B5C] py-16 px-4">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl text-white mb-4">
            Begin Your Research
            <span className="block text-[#0A8F96] italic">Journey Today</span>
          </h2>
          <p className="text-white/70 text-base mb-8 max-w-xl mx-auto">
            Apply for a PhD programme or speak to our research admissions team to explore opportunities.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/admissions/phd"
              className="inline-flex items-center justify-center gap-2 bg-[#6b4f9a] hover:bg-[#5a4182] text-white px-8 py-3.5 rounded-xl transition-all"
            >
              PhD Admissions <ArrowRight size={18} />
            </Link>
            <Link
              href="/international/apply"
...
function ProgramCard({ course }: { course: ReturnType<typeof getAllCourses>[number] }) {
  return (
    ...
    >
      {/* Top accent */}
      <div className="h-1 w-full bg-gradient-to-r from-[#6b4f9a] to-[#9b79cc]" />

      <div className="flex flex-col flex-1 p-5">
        {/* Icon */}
        <div className="w-10 h-10 rounded-xl bg-[#6b4f9a]/10 flex items-center justify-center mb-4 text-[#6b4f9a]">
        ...
        {/* Duration badge */}
        <p className="text-xs text-[#6b4f9a] bg-[#6b4f9a]/10 px-2.5 py-1 rounded-full inline-block mb-4 w-fit">
          3–5 Years
        </p>
        ...
        <div className="flex gap-2 mt-auto">
          <Link
            href={course.href}
            className="flex-1 text-center text-xs border border-[#6b4f9a] text-[#6b4f9a] py-2 rounded-lg hover:bg-[#6b4f9a]/10 transition"
          >
            View
          </Link>
          <Link
            href={applyUrlForCourse(course.name)}
            className="flex-1 text-center text-xs bg-[#6b4f9a] text-white py-2 rounded-lg hover:bg-[#5a4182] transition"
          >
            Apply
          </Link>
        </div>
