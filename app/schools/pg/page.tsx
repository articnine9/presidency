...
export default function PGProgrammesPage() {
  ...
  const pgCourses = useMemo(
    () => getAllCourses().filter((c) => c.type === "PG"),
    [],
  );

  /* ─── schools that actually have PG programmes ─── */
  const schools = useMemo(() => {
    const slugsWithPG = new Set(pgCourses.map((c) => c.schoolSlug));
    return [
      { slug: "all", label: "All Schools" },
      ...SCHOOL_ORDER.filter((s) => slugsWithPG.has(s.slug)),
    ];
  }, [pgCourses]);
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-[#f5f7fa] py-3">
        <div className="mx-auto flex max-w-[1400px] flex-wrap px-4 text-xs text-gray-600 md:px-6 md:text-sm">
          ...
          <span>Schools</span>
          <span className="mx-2 text-gray-400">›</span>
          <span className="text-[#0A8F96] font-medium">Postgraduate Programmes</span>
        </div>
      </div>
      ...
      <section className="relative w-full overflow-hidden min-h-[860px] md:min-h-screen">
        ...
        <div className="relative z-10 mx-auto flex min-h-[860px] max-w-[1400px] flex-col justify-end px-4 pb-8 pt-28 md:h-full md:min-h-screen md:justify-center md:px-14 md:pb-0">
          <div className="max-w-[560px]">
            ...
            >
              {pgCourses.length}+ postgraduate degrees across multiple schools — advance your career
              with specialised masters and MBA programmes.
            </motion.p>
              ...
              <div key={slug}>
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3">
                    <div className="w-1 h-6 rounded-full bg-[#D4A843]" />
...
function ProgramCard({ course }: { course: ReturnType<typeof getAllCourses>[number] }) {
  return (
    ...
    >
      {/* Top accent */}
      <div className="h-1 w-full bg-gradient-to-r from-[#D4A843] to-[#e8c06a]" />

      <div className="flex flex-col flex-1 p-5">
        {/* Icon */}
        <div className="w-10 h-10 rounded-xl bg-[#D4A843]/10 flex items-center justify-center mb-4 text-[#D4A843]">
          <BookOpen size={18} />
        </div>
