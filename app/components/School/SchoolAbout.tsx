...
import {
  Cpu, Globe2, Handshake, BarChart3,
  Settings, Target, Brain, Briefcase, GraduationCap,
} from "lucide-react";
...
/* ── icons pool ── */
const ICONS = [
  <Cpu size={20} />, <Globe2 size={20} />, <Handshake size={20} />,
  <BarChart3 size={20} />, <Settings size={20} />, <Target size={20} />,
  <Brain size={20} />, <Briefcase size={20} />, <GraduationCap size={20} />,
];
...
────────────────────────────────────────────────────────────── */
function HexFlower({ features, image }: { features: Feature[]; image: string }) {
  const S  = 158;                      // hex width (px)
  const H  = Math.round(S * 0.866);    // hex height
  const dH = Math.round(S * 0.75);     // horizontal step to neighbor
  const dV = Math.round(H * 0.5);      // vertical half-step to neighbor
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: canW, height: canH }}>
      {cells.map((cell, i) => (
    ...
    <section className="py-10 bg-[#F6F7FB]">
      <div className="max-w-[1400px] mx-auto px-6">
        ...
        <div className="lg:hidden space-y-6">
          <CommonHeader tag="About Us" title={data.title} primaryColor="#0A8F96" align="left" />

          <div className="space-y-3">
            {data.content.map((para, i) => (
              <p key={i} className="text-[13px] leading-7 text-gray-600 text-justify">{para}</p>
            ))}
          </div>
          ...
          {features.length > 0 && (
            <div className="grid grid-cols-2 gap-3">
              {features.slice(0, 4).map((f, i) => (
                <div key={i} className="bg-[#0A8F96] rounded-xl p-4 text-white flex flex-col items-center text-center gap-2">
          ...
          {features.length > 0 && (
            <div className="space-y-3">
              {features.map((item, i) => (
                <div key={i} className="rounded-xl border border-[#0A8F96]/20 bg-white px-4 py-4 shadow-sm">
                  <div className="flex items-start gap-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#0A8F96]/10 text-[#0A8F96]">
                      {ICONS[i % ICONS.length]}
                    </div>
                    <div>
                      <h3 className="text-[15px] font-semibold text-[#0A8F96]">{item.title}</h3>
        ...
        <div className="hidden lg:block">
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            ...
            <div>
              ...
              <div className="space-y-4">
                {data.content.map((para, i) => (
                  <p key={i} className="text-gray-600 text-base leading-relaxed text-justify">{para}</p>
                ))}
              </div>
            </div>
            ...
            <div className="flex justify-center items-center">
              {features.length >= 4 ? (
                ...
                <div className="overflow-hidden shadow-xl" style={{ width: 320, height: 277, clipPath: HEX }}>
          ...
          {features.length > 0 && (
            <div className="mt-10">
              <h2 className="text-3xl font-serif text-gray-900 mb-6">What You'll Experience Here</h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {features.map((item, i) => (
                  ...
                  >
                    ...
                    <h3 className="text-[15px] font-semibold text-[#183153] mb-1">{item.title}</h3>
