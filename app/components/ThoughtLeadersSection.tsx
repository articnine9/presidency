...
export default function ThoughtLeadersSection() {
  ...
  const Card = (item: Leader, i: number) => (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group flex min-h-[280px] flex-col bg-white rounded-xl md:min-h-[320px] md:rounded-2xl shadow-md border-b-4 border-b-[#0A8F96] overflow-hidden hover:shadow-2xl transition-all duration-300"
    >
      <div className="overflow-hidden">
        <img
          src={item.img}
          alt={item.name}
          className="h-[180px] w-full shrink-0 object-cover sm:h-[200px] md:h-[220px] group-hover:scale-110 transition duration-500"
        />
      </div>

      <div className="flex flex-1 flex-col justify-center p-3 text-center md:p-4">
